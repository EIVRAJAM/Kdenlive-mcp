...
## Audio Fade Round-Trip Verification (Pending Manual Resave)

`examples/recon/audio_fade_ai_generated.kdenlive` is generated by the MCP tools
(`export_kdenlive_timeline` -> `apply_timeline_edits` with
`fade_in_audio`/`fade_out_audio` -> `apply_timeline_to_working_project`) from
`manual_two_clips_timeline.kdenlive`. It contains a single audio clip with
`fadein window=500` and `fadeout window=400` (`mlt_service=volume`,
`kdenlive_id=fadein|fadeout`, no `level` keyframes), and loads in real melt
(`validate_project(check_mlt=True)` -> `status: loaded`, `missing_media: 0`).
...
```text
- XML well-formed and bin media still present (resource files exist)
- KdenliveProjectAdapter.extract_timeline_document reads fadein/fadeout back to
  TimelineEffect
- window_ms is preserved (expected fadein=500, fadeout=400). If Kdenlive
  transforms the window values, update the test assertion with the exact
  transformed values.
- no unexpected clip effects appear (all clip_effects must be supported)
```
