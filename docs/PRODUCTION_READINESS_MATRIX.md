...
| Security allowlists and path traversal rejection | DONE | `src/kdenlive_mcp/security.py` (`ensure_media/project/output_path`, `resolve_user_path` con `Path.resolve(strict=False)`); `tests/test_security.py` (traversal `..`, symlink dentro/fuera, allowlist vacía, y MCP `get_media_info`), `tests/test_server_protocol.py`, `tests/test_media_tools.py` | — | — |
| Stable tool response/error shapes | DONE | `server.py` `_validate_tool_arguments_schema`, `_invalid_tool_response`, inyección de `operation` y normalización de `warnings`; `tests/test_tool_response_contract.py`, `tests/test_server_protocol.py` | Ninguno mayor | — |
| Media scan and ffprobe validation | DONE | `tools/media_tools.py` (`_media_summary`, `_fps_value`, `_rotation` con tags y `side_data_list`), `adapters/ffprobe.py`; `tests/test_media_tools.py` (VFR, `avg_frame_rate` inválido, rotación, audio-only/video-only, múltiples streams, bitrate faltante) | VFR se reporta como `avg_frame_rate` crudo (string); layouts exóticos aún no probados | Ampliar mocks con más layouts si aparece evidencia real |
...
| Generic MCP client registration example | DONE | `examples/mcp_client_config.toml`, `docs/MCP_CLIENT_SETUP.md`, `examples/codex_mcp_config.toml`, `docs/CODEX_SETUP.md`; smoke test STDIO real `scripts/mcp_stdio_smoke_test.py` (`initialize` + `tools/list`, 64 tools); smoke SDK real validado con el SDK oficial `mcp` en venv aislado (`.venv`, JSONL framing, exit 0) | No probado contra todos los clientes MCP, pero el protocolo STDIO y el SDK oficial `mcp` funcionan | — |
