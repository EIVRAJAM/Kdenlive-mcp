...
| Schema documentation for persisted JSON files | DONE | `docs/SCHEMAS.md` (rough-cut plan, timeline, manifest; "Schema Migration Policy" con versiones compatibles/incompatibles y política futura), `tests/test_schema_docs.py` | Código de error de versión no soportada aún no unificado (TODO documentado) | Unificar `UNSUPPORTED_SCHEMA_VERSION` en lectores de plan/timeline/manifest |
